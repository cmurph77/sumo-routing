...
def write_new_line(filename, network, file_key, trip_size, ct,avg_tt):
    ...
    current_time = datetime.datetime.now()
    current_time = '[' + str(current_time) + '] , '
    line = str(current_time) + "Network: " + network + ", Trip Size: " + str(trip_size)+ ", Average Time: " + str(avg_tt)  + ", Algo: "  +file_key+ ", Congestion T: " + str(ct)
    with open(filename, 'a') as file:
        file.write(line + '\n')
...
def set_config_file(network,path_to_sim_files,file_key):
    ...
    # Sim output files
    output_file = "../out/"+alg_name+"_out/"+network+"_output_files/" + file_key + "_" + str(trip_count) + "tr.out.xml"
...
def read_args():
    ...
    return trip_count, network, congestion_threshold,max_vspeed
...
if __name__ == "__main__":
    ...
    trip_count, network, congestion_threshold,max_vspeed = read_args()
    ...
    config_file = set_config_file(network,path_to_sim_files,file_key)

    # Output file Locations
    congestion_matrix_output_file = out_directory+"/"+network+"_output_files/congestion_matrices/" + str(trip_count) + "tr_" + file_key + "_cm.csv"
    rel_path_output_file = out_directory+"/"+network+"_output_files/" + file_key + "_" + str(trip_count) + "tr.out.xml"
    print('Output file: '+ rel_path_output_file)
